...
def display_buy(buy_tree):
    ...
    for x in sell_result:
        ...
        ctr+=1"""
    buy_result.bind('<Double-1>', view_buy_details)

def view_buy_details(event):
    w = event.widget
    index = w.selection()[0]
    ...
    buy_bot = tk.Button(bottom_frame_buy, font=button_font_style, text="Set an Appointment", bg="#FF2E97", command=lambda:set_appointment_command(value[0])) 
...
def set_appointment_command(id_sell_land):
    window = tk.Tk()
