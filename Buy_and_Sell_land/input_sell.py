...
from listbox_sell import listbox_header
...
conn = connection()
cursor = conn.cursor()
display_basic_info="""select land_id, seller_name, land_price, province_name, contact_number from lands_for_selling;"""
create_table_sql = """
...
def add_to_sell_command():
    ...
    name_entry = tk.Entry(fillout_frame)
    ...
    tk.Button(fillout_frame, font=button_font_style, text="Confirm", command=lambda: 
              add_to_sell_db(id_value, name_entry, price_entry, address_entry, province_entry, city_entry, zip_entry,
                       contact_entry, email_entry, area_entry, status_entry, title_entry)).pack()
...
def add_to_sell_db(entry1, entry2, entry3, entry4, entry5, entry6, entry7, entry8, entry9, entry10, entry11, entry12):
    for_insert = (entry1, entry2.get(), entry3.get(), entry4.get(), entry5.get(), entry6.get(), 
    ...
    INSERT INTO lands_for_selling (land_id, seller_name, land_price, st_add, province_name, city_name, post_no,
        contact_number, email_info, size_sq_meters, title, status)
